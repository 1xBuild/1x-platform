...
const SURFACE_ERRORS = config.discord.surfaceErrors;
...
export const sendTimerMessage = async (agentId: string): Promise<string> => {
  ...
  try {
    const lettaMessage = {
      role: "user" as const,
      content: '[EVENT] This is an automated timed heartbeat (visible to yourself only). Use this event to check if you should send a message to the user to ask about their progress, or edit your memories, or do nothing at all. It\'s up to you! .'
    };

    console.log(`🛜 Sending message to Letta server (agent=${agentId}): ${JSON.stringify(lettaMessage)}`);
    const response = await lettaMessageAdapter.sendStreamMessage(agentId, lettaMessage);
    
    if (response) {
      const agentMessageResponse = await lettaMessageAdapter.processStream(response);
