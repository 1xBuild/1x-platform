...
export default function Sidebar({
  ...
  if (!isAnalystAgent) {
    sidebarItems.unshift({
      ...
      label: 'Persona',
      icon: User,
      description: 'Create a persona for your agent',
    });
  }
                ...
                className={`w-full flex items-start gap-3 p-3 rounded-lg text-left transition-colors ${
                  item.disabled
                    ...
                    : 'hover:bg-accent focus:bg-accent ' +
                      (selectedSection === item.id ? 'bg-accent' : '')
                } ${idx !== sidebarItems.length - 1 ? 'mb-2' : ''}`}
              >
                <Icon
                  className={`w-4 h-4 mt-0.5 ${item.disabled ? 'text-muted-foreground' : selectedSection === item.id ? 'text-accent-foreground' : 'text-foreground'}`}
                />
                <div className="flex-1 min-w-0">
                  <div
                    className={`font-medium text-sm ${item.disabled ? 'text-muted-foreground' : selectedSection === item.id ? 'text-accent-foreground' : 'text-foreground'}`}
                  >
                    {item.label}
                  </div>
                  {item.description && (
                    <div
                      className={`text-xs mt-0.5 ${item.disabled ? 'text-muted-foreground' : selectedSection === item.id ? 'text-accent-foreground' : 'text-muted-foreground'}`}
                    >
                      {item.description}
                    </div>
                  )}
