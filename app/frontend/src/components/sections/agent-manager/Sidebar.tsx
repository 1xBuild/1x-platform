...
export default function Sidebar({ selectedSection, setSelectedSection, agents, setSelectedAgentId, selectedAgentId }: SidebarProps) {
  ...
  const isAnalystAgent = selectedAgent?.details.name === 'analyst-agent' || selectedAgent?.details.name === 'analyst-agent-dev';
  const sidebarItems = [
  ...
  if (!isAnalystAgent) {
    sidebarItems.unshift({ id: "persona", label: "Persona", icon: User, description: "Create a persona for your agent" });
  }
